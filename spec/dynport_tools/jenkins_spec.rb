...
describe "DynportTools::Jenkins" do
  ...
  describe "#project_details" do
    ...
    it "returns a hash with the correct job details and normalizes with nokogiri" do
      ...
      yielder1 = stub.as_null_object
      yielder1.stub(:on_complete).and_yield(response1)
      yielder2 = stub.as_null_object
      yielder2.stub(:on_complete).and_yield(response2)
      Typhoeus::Request.stub(:new).with("http://hudson.host:8080/job/Job1/config.xml").and_return(yielder1)
      Typhoeus::Request.stub(:new).with("http://hudson.host:8080/job/Job2/config.xml").and_return(yielder2)

      jenkins.stub(hydra: stub.as_null_object)
